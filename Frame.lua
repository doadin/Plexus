...
function PlexusFrame.prototype:OnShow() --luacheck: ignore 212
    PlexusFrame:SendMessage("UpdateFrameUnits", "OnShow", nil, self)
...
function PlexusFrame.prototype:OnAttributeChanged(name, value)
    if name == "unit" then
        if self.unit ~= value then
            return PlexusFrame:SendMessage("UpdateFrameUnits", "OnAttributeChanged", value, self)
        end
...
PlexusFrame.defaultDB = {
    ...
    anchorAbsorbtoHealthBar = true,
    anchorHealingtoHealthBar = true,
    iconSize = 16,
    centerIconSize = 16,
    iconBorderSize = 1,
    spacingSize = 1,
    ...
    args = {
        ...
        bar = {
            ...
            args = {
                ...
                anchorAbsorbtoHealthBar = {
                    name = L["Anchor Absorb Bar to Health Bar(Requires ReloadUI)"],
                    desc = L["Anchor the absorb bar to the health bar. Disable to always show and anchor to the opposite side."],
                    order = 7, width = "double",
                    type = "toggle",
                },
                anchorHealingtoHealthBar = {
                    name = L["Anchor Incomming Healing Bar to Health Bar(Requires ReloadUI)"],
                    desc = L["Anchor the healing bar to the health bar. Disable to always show and anchor to the opposite side."],
                    order = 8, width = "double",
                    type = "toggle",
                },
            },
    ...
    if settings.enablePrivateAuraCountdownNumbers then
        return {
            ...
            iconInfo = {
                ...
                iconWidth = w,
                iconHeight = settings.PrivateAuraHeight,
                borderScale = 1.1,
            },
        }
    else
...
function PlexusFrame:UpdateFrameUnits(_, event, _, changedFrame) --self, event, unit, changedFrame
    local settings = self.db.profile
    if event == "OnAttributeChanged" or event == "OnShow" then
        local frame = changedFrame
