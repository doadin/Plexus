...
PlexusStatusRaidDebuff.menuName = "Raid Debuff"

PlexusStatusRaidDebuff.defaultDB = {
    raid_debuff = {
        enable = true,
        priority = 70,
        range = false,
...
function PlexusStatusRaidDebuff:PostInitialize()
    self:RegisterStatus("raid_debuff", "Raid Debuff", nil, true)
end

function PlexusStatusRaidDebuff:OnEnable()
...
function PlexusStatusRaidDebuff:OnDisable()
...
--function PlexusStatusRaidDebuff:LOADING_SCREEN_DISABLED()
--    PlexusStatusRaidDebuff:UpdateAllUnitsBuffs()
--end

--function PlexusStatusRaidDebuff:UpdateAllUnitsBuffs()
...
function PlexusStatusRaidDebuff:MakeContainers()
    local settings = PlexusStatusRaidDebuff.db.profile.raid_debuff
    ...
    if count == 0 then
        C_Timer.After(1, function() PlexusStatusRaidDebuff:MakeContainers() end)
        return
    end
        ...
        if frameTable.unit then
            ...
            for name, indicator in pairs(frameTable.indicators) do
                if type(indicator) == "table" and indicator.GetObjectType and indicator:GetObjectType() == "Button" then
                    ...
                    if not frameTable.container[name]:HasAuraGroup(frameName .. ":" .. name .. ":" .. "PlexusRaidDebuff") then
                        for status, statusEnabled in pairs(PlexusFrame.db.profile.statusmap[name]) do
                            if status == "raid_debuff" and statusEnabled then
                                ...
                                frameTable.container[name]:AddAuraGroup(frameName .. ":" .. name .. ":" .. "PlexusRaidDebuff", "HARMFUL|IMPORTANT", {
