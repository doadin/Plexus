...
local PlexusIndicatorsIncomingHealingBar = PlexusFrame:NewModule("PlexusIndicatorsIncomingHealingBar")
...
local function Reset(self) -- luacheck: ignore 432
    ...
    if profile.anchorHealingtoHealthBar then
        -- NEW
...
function PlexusIndicatorsIncomingHealingBar:OnInitialize() --luacheck: ignore 212
    local profile = PlexusFrame.db.profile
    if profile.enableExtraBar then
        PlexusFrame:RegisterIndicator("ei_bar_barfour", "Incoming Healing Bar", New, Reset, SetStatus, Clear)
    end
end